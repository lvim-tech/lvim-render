...
---@field box_active { first: integer, last: integer, anchor: integer, parked: integer, index: integer, rows: integer, avail: integer|nil, walk: boolean|nil, page: integer|nil }|nil
---   the WIDGET cursor: which row of a boxed table is active while the real cursor is PARKED on a
---   displayed row — `parked` (0-based) is that row: the anchor above the table when entered from
---   ABOVE, the first row below it when entered from BELOW (parking on the far-away anchor yanked
---   the view a windowful up — measured). Neovim has no mapping between a hidden buffer row, a
---   screen row inside someone else's `virt_lines`, and a topline — so walking a box is a
---   plugin-owned index, not a cursor position (see `tables_nav_mode`). `avail` is the block rows
---   the pagination may draw; `walk` true is the widget; nav mode "stop" parks the same record
---   without it, so no row lights up
...
---@field inline_pending boolean|nil  a deferred persistent-lane sync is already scheduled — the
---   decoration provider found the lane out of date but may not WRITE marks mid-redraw (line
---   heights would change while the frame is drawn); one schedule per buffer at a time
